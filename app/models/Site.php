...
class Site extends MY_Model {
    ...
    public function calculateDiscount($discount = null, $amount = null, $order = null)
    {
        if ($discount && ($order || $this->Settings->product_discount)) {
            $dpos = strpos($discount, '%');
            if ($dpos !== false) {
                $pds = explode('%', $discount);
                return $this->rerp->formatDecimal(((($this->rerp->formatDecimal($amount)) * (float) ($pds[0])) / 100), 4);
            }
            return $this->rerp->formatDecimal($discount, 4);
        }
        return 0;
    }

    public function calculateOrderTax( $order_tax_id = null, $amount = null, $format = true ) {
	    if ( $this->Settings->tax2 != 0 && $order_tax_id ) {
	        if ( $order_tax_details = $this->site->getTaxRateByID( $order_tax_id ) ) {
                if ($order_tax_details->type == 1) {
	                $ot = ( ( $amount * $order_tax_details->rate ) / 100 );
                    return ! $format ? $ot : $this->rerp->formatDecimal( $ot , 4 );
                }
                return ! $format ? $order_tax_details->rate : $this->rerp->formatDecimal( $order_tax_details->rate, 4 );
            }
        }
        return 0;
    }
	 ...
	 * @param object $tax_details
	 * @param float|int $custom_value
	 * @param string $c_on
	 *
	 * @return array
	 */
    public function calculateTax($product_details = null, $tax_details = null, $custom_value = null, $c_on = null)
    {
        ...
        if ($tax_details && $tax_details->type == 1 && $tax_details->rate != 0) {
            if ($product_details && $product_details->tax_method == 1) {
                $tax_amount = $this->rerp->formatDecimal((($value) * $tax_details->rate) / 100, 4);
                $tax        = $this->rerp->formatDecimal($tax_details->rate, 0) . '%';
            } else {
                $tax_amount = $this->rerp->formatDecimal((($value) * $tax_details->rate) / (100 + $tax_details->rate), 4);
                $tax        = $this->rerp->formatDecimal($tax_details->rate, 0) . '%';
            }
        } elseif ($tax_details && $tax_details->type == 2) {
            $tax_amount = $this->rerp->formatDecimal($tax_details->rate);
            $tax        = $this->rerp->formatDecimal($tax_details->rate, 0);
        }
        return ['id' => $tax_details->id, 'tax' => $tax, 'amount' => $tax_amount];
    }
    ...
    public function costing($items)
    {
                    ...
                    if ($combo_item->type == 'standard') {
                        if (isset($citems['p' . $combo_item->id . 'o' . $item['option_id']])) {
	                        ...
	                        if ( $cpr->tax_rate ) {
                                ...
                                if ($cpr->tax_method) {
                                    $item_tax       = $this->rerp->formatDecimal((($combo_item->unit_price) * $cpr_tax->rate) / (100 + $cpr_tax->rate));
                                    $net_unit_price = $combo_item->unit_price - $item_tax;
                                    $unit_price     = $combo_item->unit_price;
                                } else {
                                    $item_tax       = $this->rerp->formatDecimal((($combo_item->unit_price) * $cpr_tax->rate) / 100);
        ...
        // $this->rerp->print_arrays($combo_items, $citems);
        $cost = [];
    ...
    {
        $orderby = empty($this->Settings->accounting_method) ? 'asc' : 'desc';
        $this->db->select('id, purchase_id, quantity, quantity_balance, net_unit_cost, unit_cost, item_tax');
	...
	public function checkUserNameExists( $username ) {
		...
		return $this->db->get( 'users', 1 )->row() !== null;
	}
                ...
                if ($item['product_type'] == 'standard') {
                    ...
                    foreach ($combo_items as $combo_item) {
                        ...
                        if ($pr->tax_rate) {
                            ...
                            if ($pr->tax_method) {
                                $item_tax       = $this->rerp->formatDecimal((($combo_item->unit_price) * $pr_tax->rate) / (100 + $pr_tax->rate));
                                $net_unit_price = $combo_item->unit_price - $item_tax;
                                $unit_price     = $combo_item->unit_price;
                            } else {
                                $item_tax       = $this->rerp->formatDecimal((($combo_item->unit_price) * $pr_tax->rate) / 100);
                                $net_unit_price = $combo_item->unit_price;
                                $unit_price     = $combo_item->unit_price + $item_tax;
                            }
                        } else {
    ...
    public function setPurchaseItem($clause, $qty)
    {
        if ($product = $this->getProductByID($clause['product_id'])) {
            ...
            if (isset($product->tax_rate) && $product->tax_rate != 0) {
                ...
                $pr_item_tax = $this->rerp->formatDecimal($item_tax * $clause['unit_quantity'], 4);
    ...
    public function syncPurchasePayments($id)
    {
        ...
        if ($this->rerp->formatDecimal($purchase->grand_total) > $this->rerp->formatDecimal($paid) && $paid > 0) {
            $payment_status = 'partial';
        } elseif ($this->rerp->formatDecimal($purchase->grand_total) <= $this->rerp->formatDecimal($paid)) {
            $payment_status = 'paid';
        }
	...
	public function syncSalePayments( $id ) {
		...
		if ( $payments = $this->getSalePayments( $id ) ) {
			...
			if ( $this->rerp->formatDecimal( $grand_total ) == 0 || $this->rerp->formatDecimal( $grand_total ) <= $this->rerp->formatDecimal( $paid ) ) {
				$payment_status = 'paid';
			...
			$updateSale = $this->db->update( 'sales', [ 'paid' => $paid, 'payment_status' => $payment_status ], [ 'id' => $id ] );
			if ( $updateSale ) {
				// handle commission & cash-back only if payment is 100 %.
				if ( 'paid' === $payment_status ) {
					self::calculate_cash_back_and_commission( $id );
					$this->create_delivery( $id, $sale );
				}
				return true;
			}
        } else {
    ...
    protected function create_delivery( $id, $sale ) {
		$need_delivery = $sale->shop || $sale->api;
	    if (
	    	! $this->Erp_Options->getOption( '_auto_create_delivery', 0 ) ||
		    $sale->sale_status !== 'completed' || ! $need_delivery
	    ) {
	    	return;
	    }
	    // check if anyone already created delivery for this sales.
	    $deliveries = $this->db
		    ->where( 'sale_id', $id )
		    ->or_where( 'sale_reference_no', $sale->reference_no )
	    ...
	    $address  = $this->site->getAddressByID( $sale->address_id );
	    $data     = [
		    ...
		    'received_by'       => null,
		    'attachment'        => null,
		    'note'              => $sale->note ? '<p>Customer\'s Order Note: ' . strip_tags( $sale->note ) . '</p>' : null,
	    ];
	    ...
	    if ( $this->shop_model->hasShippingSlot() && ( $schedule = $this->sales_model->getDeliveryScheduleBySaleId( $id ) ) ) {
		    ...
		    $data['date'] = $schedule->start;
	    }
	
	    if ( $delivery_id = $this->sales_model->addDelivery( $data )) {
		
		    $this->load->library( 'sms' );
		    $this->sms->delivering( $sale->id, $data['do_reference_no'] );
