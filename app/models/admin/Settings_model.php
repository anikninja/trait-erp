...
class Settings_model extends MY_Model
{
	...
	public function addShippingZone( $data ) {
	...
	public function addUnits( $baseUnits, $subUnits ) {
		$result = false;
		
		if ( ! empty( $baseUnits ) ) {
			foreach ( $baseUnits as $baseUnit ) {
				if ( ! is_int( $baseUnit['base_unit'] ) ) {
					$baseUnit['base_unit'] = null;
				}
				$this->db->insert( 'units', $baseUnit );
			}
			$result = true;
		}
		...
		if ( ! empty( $subUnits ) ) {
			foreach ( $subUnits as $subUnit ) {
				if ( ! is_int( $subUnit['base_unit'] ) ) {
					if ( $baseUnit = $this->getUnitByCode( $subUnit['base_unit'] ) ) {
						$subUnit['base_unit'] = $baseUnit->id;
						
					}
				}
				
				$this->db->insert( 'units', $subUnit );
			}
			$result = true;
		}
	...
	public function deleteShippingZone( $id ) {
		if ( $this->db->delete( 'shipping_zones', [ 'id' => $id ] ) ) {
            return true;
        }
        return false;
    }
	...
	public function getShippingZoneByID( $id ) {
		$q = $this->db->get_where( 'shipping_zones', [ 'id' => $id ], 1 );
		if ( $q->num_rows() > 0 ) {
			return $q->row();
		}
	...
	public function updateShippingZone( $id, $data = [] ) {
		$this->db->where('id', $id);
		if ( $this->db->update( 'shipping_zones', $data ) ) {
			return true;
		}
		return false;
	}
	...
	public function updateSslcommerz($data)
	{
		$this->db->where('id', '1');
		if ($this->db->update('sslcommerz', $data)) {
			return true;
		}
		return false;
	}
