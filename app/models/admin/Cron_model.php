...
class Cron_model extends CI_Model
{
        ...
        if ($details) {
            ...
            foreach ($owners as $owner) {
                ...
                if ($domain != 'retailpremier.com') {
                    ...
                    $msg     = file_get_contents('./themes/' . $this->Settings->adminTheme . 'email_templates/cron.html');
    ...
    private function checkUpdate()
    {
        ...
        $updates  = get_remote_contents($protocol . 'retailpremier.com/api/v1/update/', $fields);
    ...
    private function gen_html($costing, $discount, $expenses, $returns, $purchases, $sales, $warehouse = null)
    {
        ...
        <tr>
            ...
            <td style="text-align:right; border-bottom: 1px solid #EEE;">' . $this->rerp->formatMoney($costing->sales) . '</td>
        </tr>';
        if ($discount && $discount->order_discount > 0) {
            $html .= '
            <tr>
                <td style="border-bottom: 1px solid #DDD;">' . lang('order_discount') . '</td>
                <td style="text-align:right;border-bottom: 1px solid #DDD;">' . $this->rerp->formatMoney($discount->order_discount) . '</td>
            </tr>';
        }
        $html .= '
        <tr>
            <td style="border-bottom: 1px solid #EEE;">' . lang('products_cost') . '</td>
            <td style="text-align:right; border-bottom: 1px solid #EEE;">' . $this->rerp->formatMoney($costing->cost) . '</td>
        </tr>';
        if ($expenses && $expenses->total > 0) {
            $html .= '
            <tr>
                <td style="border-bottom: 1px solid #DDD;">' . lang('expenses') . '</td>
                <td style="text-align:right;border-bottom: 1px solid #DDD;">' . $this->rerp->formatMoney($expenses->total) . '</td>
            </tr>';
        }
        $html .= '
        <tr>
            ...
            <td style="text-align:right;border-bottom: 1px solid #DDD;">
                <strong>' . $this->rerp->formatMoney($costing->sales - $costing->cost - ($discount ? $discount->order_discount : 0) - ($expenses ? $expenses->total : 0)) . '</strong>
            </td>
        </tr>';
        if (isset($returns->total)) {
            ...
            <tr>
                ...
                <td style="text-align:right;border-bottom: 2px solid #DDD;"><strong>' . $this->rerp->formatMoney($returns->total) . '</strong></td>
            </tr>';
        }
        ...
        if ($sales) {
            $html .= '
            <tr>
                <td width="33%" style="border-bottom: 1px solid #DDD;">' . lang('total_sales') . ': <strong>' . $this->rerp->formatMoney($sales->total_amount) . '(' . $sales->total . ')</strong></td>
                <td width="33%" style="border-bottom: 1px solid #DDD;">' . lang('received') . ': <strong>' . $this->rerp->formatMoney($sales->paid) . '</strong></td>
                <td width="33%" style="border-bottom: 1px solid #DDD;">' . lang('taxes') . ': <strong>' . $this->rerp->formatMoney($sales->tax) . '</strong></td>
            </tr>';
        }
        if ($purchases) {
            $html .= '
            <tr>
                <td width="33%">' . lang('total_purchases') . ': <strong>' . $this->rerp->formatMoney($purchases->total_amount) . '(' . $purchases->total . ')</strong></td>
                <td width="33%">' . lang('paid') . ': <strong>' . $this->rerp->formatMoney($purchases->paid) . '</strong></td>
                <td width="33%">' . lang('taxes') . ': <strong>' . $this->rerp->formatMoney($purchases->tax) . '</strong></td>
            </tr>';
        }
        $html .= '</table></div>';
        return $html;
