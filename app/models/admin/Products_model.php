...
class Products_model extends MY_Model {
		...
		if ( $this->db->insert( 'products', $data ) ) {
            ...
            if ($product_attributes) {
                foreach ($product_attributes as $pr_attr) {
	                ...
	                $pr_attr['quantity'] = absint( $pr_attr['quantity'] );
    ...
    public function updateProductImage( $id, $image ) {
	    return $this->db->update( 'products', [ 'image' => $image ], [ 'id' => $id ] );
    }
	
	/**
	 * Insert New Additional Image for product.
	 * @param int $id
	 * @param string $image
	 *
	 * @return bool|int
	 */
    public function addProductAdditionalImage( $id, $image ) {
		if ( $this->db->insert( 'product_photos', [ 'product_id' => $id, 'photo' => $image ] ) ) {
	...
	/**
	 * Remove Product Additional Image by id.
	 * @param int $id
	 * @param int $image
	 *
	 * @return bool|int
	 */
    public function deleteProductAdditionalImage( $id, $image ) {
    	$this->db->delete( 'product_photos', [ 'product_id' => $id, 'id' => $image ] );
