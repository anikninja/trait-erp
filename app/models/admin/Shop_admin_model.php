...
/**
 * Class Shop_admin_model
 * @property CI_DB_mysqli_driver $db
 */
class Shop_admin_model extends My_Model
{
    ...
    public function getColorsFromVariations() {
	    $results = $this->db
		    ->select('name')
	        ->like( 'name', 'color:' )
	    ...
	    foreach ($results as $row ) {
		    ...
		    foreach ( $parts as $part ) {
		    	...
		    	if ( 'color' === strtolower( $name ) && $value ) {
		    		$k = preg_replace( '/[^a-z0-9]/', '', strtolower( $value ) );
		    		$data[ $k ] = $value;
			    }
		    }
	    }
