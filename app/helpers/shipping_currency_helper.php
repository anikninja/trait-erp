...
if ( ! function_exists( 'ci_get_currencies' ) ) {
	/**
...
if ( ! function_exists( 'ci_currency_symbols' ) ) {
	/**
	 * Get Currency symbol.
	 * @woocommerce
	 *
...
if ( ! function_exists( 'ci_currency_symbol' ) ) {
	/**
	 * Get Currency symbol.
	 * @woocommerce
	 *
	 ...
	 * @return string
	 */
	function ci_currency_symbol( $currency = '' ) {
		$symbols = ci_currency_symbols();
		return isset( $symbols[ $currency ] ) ? $symbols[ $currency ] : '';
	}
}

if ( ! function_exists( 'ci_get_continents' ) ) {
	/**
	 * Get continents
	 * @param string $cc        Continent code to get country codes of.
	 * @param bool   $cc_labels Get labels with code.
	 *
	 * @return array
	 */
	function ci_get_continents( $cc = null, $cc_labels = true ) {
		$continents = [
			'AF' => [
				'name'      => 'Africa',
				'countries' => [
		...
		if ( 'names' === $cc ) {
			...
			foreach ( $continents as $k => $continent ) {
				$temp[ $k ] = $continent['name'];
			}
			
			return $temp;
		} else {
			$cc = strtoupper( $cc );
			if ( isset( $continents[ $cc ] ) ) {
				if ( ! $cc_labels ) {
					return $continents[ $cc ]['countries'];
				}
				return array_intersect_key( ci_get_countries(), array_flip( $continents[ $cc ]['countries'] ) );
			}
		}
	}
}
if ( ! function_exists( 'ci_get_countries' ) ) {
	function ci_get_countries( $index = null ) {
		$return = [
			'AF' => 'Afghanistan',
			'AX' => '&#197;land Islands',
			'AL' => 'Albania',
...
if ( ! function_exists( 'ci_get_states' ) ) {
	function ci_get_states( $cc = null, $sc = null ) {
		$states = [
			'AF' => [],
			'AO' => [ // Angola states.
				'BGO' => 'Bengo',
		...
		if ( ! is_null( $cc ) ) {
			if( ! is_null( $sc ) ) {
				return isset( $states[ $cc ][ $sc ] ) ? $states[ $cc ][ $sc ] : false;
			}
...
if ( ! function_exists( 'ci_get_country_calling_code' ) ) {
	function ci_get_country_calling_code( $cc = null ) {
		$codes = [
			'BD' => '+880',
			'BE' => '+32',
			'BF' => '+226',
		...
		if ( ! is_null( $cc ) ) {
			return isset( $codes[ $cc ] ) ? $codes[ $cc ] : false;
		}
		
		return $codes;
	}
}
if ( ! function_exists( 'ci_get_shipping_methods' ) ) {
	function ci_get_shipping_methods( $method = null ) {
		$methods = [
			'flat_rate_shipping' => lang( 'flat_rate_shipping' ),
			'free_shipping'      => lang( 'free_shipping' ),
			'local_pickup'       => lang( 'local_pickup' ),
		];
