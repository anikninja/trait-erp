...
class Pos extends MY_Controller
{
    ...
    {
        $this->rerp->checkPermissions('payments', true, 'sales');
    ...
    public function ajaxbranddata($brand_id = null)
    {
        ...
        $this->rerp->send_json(['products' => $products, 'tcp' => $tcp]);
    }
    ...
    public function ajaxcategorydata($category_id = null)
    {
	    ...
	    if ( $subcategories ) {
		    foreach ( $subcategories as $category ) {
	            $image = $this->getThumb( $category->image );
			    $scats .= sprintf(
		            '<button id="subcategory-%s" type="button" value="%s" class="btn-prni subcategory" ><img src="%s" class="img-rounded img-thumbnail"><span>%s</span></button>',
		            $category->id,
		            $category->id,
		            $image,
		            $category->name
        ...
        $this->rerp->send_json(['products' => $products, 'subcategories' => $scats, 'tcp' => $tcp]);
    }
    ...
    public function check_pin()
    {
        ...
        if ($pin == $this->pos_pin) {
            $this->rerp->send_json(['res' => 1]);
        }
        $this->rerp->send_json(['res' => 0]);
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->pos_model->deleteBill($id)) {
            $this->rerp->send_json(['error' => 0, 'msg' => lang('suspended_sale_deleted')]);
        }
    }
    ...
    public function delete_printer($id = null)
    {
        ...
        if ($this->pos_model->deletePrinter($id)) {
            $this->rerp->send_json(['error' => 0, 'msg' => lang('printer_deleted')]);
        }
    }
    ...
    public function email_receipt($sale_id = null, $view = null)
    {
        ...
        if (!$to) {
            $this->rerp->send_json(['msg' => $this->lang->line('no_meil_provided')]);
        }

        try {
            if ($this->rerp->send_email($to, lang('receipt_from') . ' ' . $this->data['biller']->company, $receipt)) {
                $this->rerp->send_json(['msg' => $this->lang->line('email_sent')]);
            } else {
                $this->rerp->send_json(['msg' => $this->lang->line('email_failed')]);
            }
        } catch (Exception $e) {
            $this->rerp->send_json(['msg' => $e->getMessage()]);
        }
    }
    ...
    public function index($sid = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $note             = $this->rerp->clear_tags($this->input->post('pos_note'));
            ...
            if ($this->pos_settings->rounding) {
                $round_total = $this->rerp->roundNumber($grand_total, $this->pos_settings->rounding);
                $rounding    = $this->rerp->formatMoney($round_total - $grand_total);
            }
            $data = ['date'         => $date,
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        $amount = $this->rerp->formatDecimal($_POST['balance_amount'][$r] > 0 ? $_POST['amount'][$r] - $_POST['balance_amount'][$r] : $_POST['amount'][$r]);
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if (($sid || $duplicate_sale) && $inv_items) {
                ...
                foreach ($inv_items as $item) {
                    ...
                    $row->price           = $this->rerp->formatDecimal($item->net_unit_price + $this->rerp->formatDecimal($item->item_discount / $item->quantity));
                    $row->unit_price      = $row->tax_method ? $item->unit_price + $this->rerp->formatDecimal($item->item_discount / $item->quantity) + $this->rerp->formatDecimal($item->item_tax / $item->quantity) : $item->unit_price + ($item->item_discount / $item->quantity);
            ...
            // $this->rerp->print_arrays($this->data['items']);
            } else {
    ...
    public function opened_bills($per_page = 0)
    {
        ...
        if (!empty($bills)) {
            ...
            foreach ($bills as $bill) {
                $html .= '<li><button type="button" class="btn btn-info sus_sale" id="' . $bill->id . '"><p>' . $bill->suspend_note . '</p><strong>' . $bill->customer . '</strong><br>' . lang('date') . ': ' . $bill->date . '<br>' . lang('items') . ': ' . $bill->count . '<br>' . lang('total') . ': ' . $this->rerp->formatMoney($bill->total) . '</button></li>';
            }
            $html .= '</ul>';
        } else {
