...
class Transfers extends MY_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_expiry        = isset($_POST['expiry'][$r]) ? $this->rerp->fsd($_POST['expiry'][$r]) : null;
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    $product = [
                        ...
                        'unit_cost'         => $this->rerp->formatDecimal($item_net_cost + $item_tax, 4),
            ...
            $grand_total = $this->rerp->formatDecimal(($total + $shipping + $product_tax), 4);
    ...
    public function combine_pdf($transfers_id)
    {
        ...
        foreach ($transfers_id as $transfer_id) {
            ...
            if (!$this->session->userdata('view_right')) {
                $this->rerp->view_rights($transfer->created_by);
            }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->transfers_model->deleteTransfer($id)) {
            if ($this->input->is_ajax_request()) {
                $this->rerp->send_json(['error' => 0, 'msg' => lang('transfer_deleted')]);
            }
    ...
    public function email($transfer_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $transfer_temp = file_get_contents('themes/' . $this->Settings->adminTheme . 'email_templates/transfer.html');
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->rerp->hrld($tansfer->date));
            ...
            if (isset($_FILES['userfile'])) {
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($item_code) && isset($unit_cost) && isset($item_quantity)) {
                        ...
                        $product_tax += $pr_item_tax;
                        $subtotal = $this->rerp->formatDecimal((($item_net_cost * $item_quantity) + $pr_item_tax), 4);
                        ...
                        $product = [
                            ...
                            'net_unit_cost'     => $item_net_cost,
                            'unit_cost'         => $this->rerp->formatDecimal($unit_cost, 4),
                            'quantity'          => $item_quantity,
    ...
    public function view($transfer_id = null)
    {
        ...
        $transfer            = $this->transfers_model->getTransferByID($transfer_id);
        if (!$this->session->userdata('view_right')) {
            $this->rerp->view_rights($transfer->created_by, true);
        }
        $this->data['rows']           = $this->transfers_model->getAllTransferItems($transfer_id, $transfer->status);
        $this->data['from_warehouse'] = $this->site->getWarehouseByID($transfer->from_warehouse_id);
        $this->data['to_warehouse']   = $this->site->getWarehouseByID($transfer->to_warehouse_id);
