...
class Purchases extends MY_Controller
{
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_net_cost      = $this->rerp->formatDecimal($_POST['net_cost'][$r]);
                $unit_cost          = $this->rerp->formatDecimal($_POST['unit_cost'][$r]);
                $real_unit_cost     = $this->rerp->formatDecimal($_POST['real_unit_cost'][$r]);
                $item_unit_quantity = $_POST['quantity'][$r];
                $item_option        = isset($_POST['product_option'][$r]) && $_POST['product_option'][$r] != 'false' ? $_POST['product_option'][$r] : null;
                $item_tax_rate      = $_POST['product_tax'][$r]      ?? null;
                $item_discount      = $_POST['product_discount'][$r] ?? null;
                $item_expiry        = (isset($_POST['expiry'][$r]) && !empty($_POST['expiry'][$r])) ? $this->rerp->fsd($_POST['expiry'][$r]) : null;
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    $unit_cost        = $this->rerp->formatDecimal($unit_cost - $pr_discount);
                    ...
                    $product = [
                        ...
                        'unit_cost'         => $this->rerp->formatDecimal($item_net_cost + $item_tax),
                    ...
                    $total += $this->rerp->formatDecimal(($item_net_cost * $item_unit_quantity), 4);
                }
            }
            if (empty($products)) {
    ...
    {
        $this->rerp->checkPermissions('expenses', true);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->purchases_model->deletePurchase($id)) {
            if ($this->input->is_ajax_request()) {
                $this->rerp->send_json(['error' => 0, 'msg' => lang('purchase_deleted')]);
            }
    ...
    public function delete_expense($id = null)
    {
        ...
        if ($this->purchases_model->deleteExpense($id)) {
            ...
            $this->rerp->send_json(['error' => 0, 'msg' => lang('expense_deleted')]);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            foreach ($inv_items as $item) {
                ...
                $row->expiry           = (($item->expiry && $item->expiry != '0000-00-00') ? $this->rerp->hrsd($item->expiry) : '');
                $row->base_quantity    = $item->quantity;
                $row->base_unit        = $row->unit ? $row->unit : $item->product_unit_id;
                $row->base_unit_cost   = $row->cost ? $row->cost : $item->unit_cost;
                $row->unit             = $item->product_unit_id;
                ...
                $row->cost             = $this->rerp->formatDecimal($item->net_unit_cost + ($item->item_discount / $item->quantity));
                $row->tax_rate         = $item->tax_rate_id;
    ...
    public function email($purchase_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $purchase_temp = file_get_contents('themes/' . $this->Settings->adminTheme . 'email_templates/purchase.html');
    ...
    public function email_payment($id = null)
    {
        ...
        if (!$supplier->email) {
            $this->rerp->send_json(['msg' => lang('update_supplier_email')]);
        }
        ...
        if ($this->rerp->send_email($supplier->email, $subject, $message)) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->rerp->hrld($expense->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $expense->reference);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->rerp->formatMoney($expense->amount));
    ...
    public function pdf($purchase_id = null, $view = null, $save_bufffer = null)
    {
        ...
        if ($view) {
            ...
            return $this->rerp->generate_pdf($html, $name, $save_bufffer);
        }
        $this->rerp->generate_pdf($html, $name);
    }
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        $purchase = $this->purchases_model->getPurchaseByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->rerp->hrld($purchase->date));
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->rerp->formatMoney($purchase->grand_total));
                        $row++;
                    }
                ...
                foreach ($final as $csv_pr) {
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_cost']) && isset($csv_pr['quantity'])) {
                        if ($product_details = $this->purchases_model->getProductByCode($csv_pr['code'])) {
                            ...
                            $item_net_cost    = $this->rerp->formatDecimal($csv_pr['net_unit_cost']);
                            $item_quantity    = $csv_pr['quantity'];
                            $quantity_balance = $csv_pr['quantity'];
                            $item_tax_rate    = $csv_pr['item_tax_rate'];
                            $item_discount    = $csv_pr['discount'];
                            $item_expiry      = isset($csv_pr['expiry']) ? $this->rerp->fsd($csv_pr['expiry']) : null;
                            ...
                            $product_tax += $pr_item_tax;
                            $subtotal = $this->rerp->formatDecimal(($unit_cost * $item_quantity), 4);
                            ...
                            $product  = [
                                ...
                                'date'              => date('Y-m-d', strtotime($date)),
                                'status'            => $status,
                                'unit_cost'         => $unit_cost, // $this->rerp->formatDecimal(($item_net_cost + $item_tax), 4),
                                'real_unit_cost'    => $this->rerp->formatDecimal(($unit_cost + $pr_discount), 4),
                            ];

                            $products[] = ($product + $gst_data);
                            $total += $this->rerp->formatDecimal(($item_net_cost * $item_quantity), 4);
                        } else {
    ...
    {
        $this->rerp->checkPermissions('return_purchases');
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = $this->rerp->formatDecimal((($item_net_cost * $item_unit_quantity) + $pr_item_tax), 4);
                    $unit     = $this->site->getUnitByID($item_unit);
            ...
            $order_tax      = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
            $total_tax      = $this->rerp->formatDecimal(($product_tax + $order_tax), 4);
            $grand_total    = $this->rerp->formatDecimal(($this->rerp->formatDecimal($total) + $this->rerp->formatDecimal($total_tax) + $this->rerp->formatDecimal($return_surcharge) - $this->rerp->formatDecimal($order_discount)), 4);
            $data           = ['date' => $date,
                'purchase_id'         => $id,
