...
class Customers extends MY_Controller
{
	...
	public function add_manually()
	{
		...
		if ( $this->form_validation->run() == true ) {
			...
			$password       = ci_generate_password(6 );
		...
		$register = null;
		if ( $this->form_validation->run() == true && $register = $this->ion_auth->register($username, $password, $email, $additional_data, 1, 1) ) {
			...
			$this->data['customer_groups'] = $this->companies_model->getAllCustomerGroups();
			$this->data['price_groups']    = $this->companies_model->getAllPriceGroups();
			$this->load->view($this->theme . 'customers/add_manually', $this->data);
		}
	}
	// ===== Some methods are needed to copy from Main.php ===== //
    ...
    {
        $this->rerp->checkPermissions('add', true);
    ...
    {
        $this->rerp->checkPermissions('deposits', true);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->input->get('id') == 1) {
            $this->rerp->send_json(['error' => 1, 'msg' => lang('customer_x_deleted')]);
        }

        if ($this->companies_model->deleteCustomer($id)) {
            $this->rerp->send_json(['error' => 0, 'msg' => lang('customer_deleted')]);
        } else {
            $this->rerp->send_json(['error' => 1, 'msg' => lang('customer_x_deleted_have_sales')]);
        }
    }
    ...
    public function delete_deposit($id)
    {
        ...
        if ($this->companies_model->deleteDeposit($id)) {
            $this->rerp->send_json(['error' => 0, 'msg' => lang('deposit_deleted')]);
        }
    }
    ...
    public function get_award_points($id = null)
    {
        ...
        $this->rerp->send_json(['ca_points' => $row->award_points]);
    }

    public function get_customer_details($id = null)
    {
        $this->rerp->send_json($this->companies_model->getCompanyByID($id));
    }

    public function get_deposits($company_id = null)
    {
        $this->rerp->checkPermissions('deposits');
    ...
    public function getCustomer($id = null)
    {
        // $this->rerp->checkPermissions('index');
        $row = $this->companies_model->getCompanyByID($id);
        $this->rerp->send_json([['id' => $row->id, 'text' => ($row->company && $row->company != '-' ? $row->company : $row->name)]]);
    }
    ...
    public function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES['csv_file'])) {
                ...
                // $this->rerp->print_arrays($data, $updated);
            }
