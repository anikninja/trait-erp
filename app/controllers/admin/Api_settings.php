...
class Api_settings extends MY_Controller
{
    ...
    {
        if (DEMO) {
            $this->rerp->send_json(['error' => 1, 'msg' => lang('disabled_in_demo')]);
        }

        if ($this->api_model->deleteApiKey($id)) {
            $this->rerp->send_json(['error' => 0, 'msg' => lang('api_key_deleted')]);
        } else {
            $this->rerp->send_json(['error' => 1, 'msg' => lang('delete_failed')]);
        }
    }
