...
class Sales extends MY_Controller
{
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $staff_note       = $this->rerp->clear_tags($this->input->post('staff_note'));
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $real_unit_price    = $this->rerp->formatDecimal($_POST['real_unit_price'][$r]);
                $unit_price         = $this->rerp->formatDecimal($_POST['unit_price'][$r]);
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $unit_price       = $this->rerp->formatDecimal($unit_price - $pr_discount);
                    $item_net_price   = $unit_price;
                    $pr_item_discount = $this->rerp->formatDecimal($pr_discount * $item_unit_quantity);
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        ...
                        $item_tax    = $this->rerp->formatDecimal($ctax['amount']);
                        $tax         = $ctax['tax'];
            ...
            $total_discount = $this->rerp->formatDecimal(($order_discount + $product_discount), 4);
            $order_tax      = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
            $total_tax      = $this->rerp->formatDecimal(($product_tax + $order_tax), 4);
            $grand_total    = $this->rerp->formatDecimal(($this->rerp->formatDecimal($total) + $this->rerp->formatDecimal($total_tax) + $this->rerp->formatDecimal($shipping) - $this->rerp->formatDecimal($order_discount)), 4);
            $data           = ['date' => $date,
                ...
                'order_tax'           => $order_tax,
                'total_tax'           => $total_tax,
                'shipping'            => $this->rerp->formatDecimal($shipping),
            ...
            if ($payment_status == 'partial' || $payment_status == 'paid') {
                ...
                if ($this->input->post('paid_by') == 'gift_card') {
                    ...
                    $payment       = [
                        ...
                        'amount'       => $this->rerp->formatDecimal($amount_paying),
                    ...
                    $payment = [
                        'date'         => $date,
                        'reference_no' => $this->input->post('payment_reference_no'),
                        'amount'       => $this->rerp->formatDecimal($this->input->post('amount-paid')),
                        'paid_by'      => $this->input->post('paid_by'),
                        'cheque_no'    => $this->input->post('cheque_no'),
            ...
            // $this->rerp->print_arrays($data, $products, $payment);
        }
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment)) {
            ...
            if ($quote_id || $sale_id) {
                ...
                foreach ($items as $item) {
                    ...
                    $row->price           = $this->rerp->formatDecimal($item->net_unit_price + $this->rerp->formatDecimal($row->item_discount));
                    $row->unit_price      = $row->tax_method ? $item->unit_price + $this->rerp->formatDecimal($row->item_discount) + $this->rerp->formatDecimal($row->item_tax) : $item->unit_price + ($row->item_discount);
    ...
    public function add_delivery($id = null)
    {
	    ...
	    if ( $delivery = $this->sales_model->getDeliveryBySaleID( $id ) ) {
		    ...
		    if ( $this->form_validation->run() == true && $delivery_id = $this->sales_model->addDelivery( $dlDetails ) ) {
			    ...
			    admin_redirect( 'delivery/delivery_list' );
            } else {
		    	$scheduled = false;
			    if ( $this->shop_model->hasShippingSlot() && ( $schedule = $this->sales_model->getDeliveryScheduleBySaleId( $id ) ) ) {
				    $scheduled = true;
    ...
    {
        $this->rerp->checkPermissions(false, true);
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data             = ['card_no' => $this->input->post('card_no'),
                ...
                'expiry'                   => $this->input->post('expiry') ? $this->rerp->fsd($this->input->post('expiry')) : null,
                'created_by'               => $this->session->userdata('user_id'),
            ];
            $sa_data = [];
            $ca_data = [];
            ...
            // $this->rerp->print_arrays($data, $ca_data, $sa_data);
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            //$this->rerp->print_arrays($payment);
    ...
    {
        $this->rerp->checkPermissions('pdf');

        foreach ($sales_id as $id) {
            $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
            $inv                 = $this->sales_model->getInvoiceByID($id);
            if (!$this->session->userdata('view_right')) {
                $this->rerp->view_rights($inv->created_by);
            }
    ...
    public function delete($id = null)
    {
        ...
        if ($inv->sale_status == 'returned') {
            $this->rerp->send_json(['error' => 1, 'msg' => lang('sale_x_action')]);
        }

        if ($this->sales_model->deleteSale($id)) {
            if ($this->input->is_ajax_request()) {
                $this->rerp->send_json(['error' => 0, 'msg' => lang('sale_deleted')]);
            }
    ...
    public function delete_delivery($id = null)
    {
        ...
        if ($this->sales_model->deleteDelivery($id)) {
            $this->rerp->send_json(['error' => 0, 'msg' => lang('delivery_deleted')]);
        }
    }
    ...
    public function delete_gift_card($id = null)
    {
        ...
        if ($this->sales_model->deleteGiftCard($id)) {
            $this->rerp->send_json(['error' => 0, 'msg' => lang('gift_card_deleted')]);
        }
    }
    ...
    public function delete_return($id = null)
    {
        ...
        if ($this->sales_model->deleteReturn($id)) {
            if ($this->input->is_ajax_request()) {
                $this->rerp->send_json(['error' => 0, 'msg' => lang('return_sale_deleted')]);
            }
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    $this->rerp->checkPermissions('delete_delivery');
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->rerp->hrld($delivery->date));
    ...
    public function email($id = null)
    {
	    ...
	    if ( $this->form_validation->run() == true ) {
            ...
            try {
	            if ( $this->rerp->send_email( $to, $subject, $message, null, null, $attachment, $cc, $bcc ) ) {
		    ...
		    $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata( 'error' );
            $sale_temp = file_get_contents('themes/' . $this->Settings->adminTheme . 'email_templates/sale.html');
    ...
    public function email_payment($id = null)
    {
        ...
        $customer             = $this->site->getCompanyByID($inv->customer_id);
        if (!$customer->email) {
            $this->rerp->send_json(['msg' => lang('update_customer_email')]);
        }
        $this->data['inv']        = $inv;
        $this->data['payment']    = $payment;
        ...
        $msg     = file_get_contents('./themes/' . $this->Settings->adminTheme . 'email_templates/email_con.html');
        $message = $this->parser->parse_string($msg, $parse_data);
        $subject = lang('payment_note') . ' - ' . $this->Settings->site_name;

        if ($this->rerp->send_email($customer->email, $subject, $message)) {
            $this->rerp->send_json(['msg' => lang('email_sent')]);
        } else {
            $this->rerp->send_json(['msg' => lang('email_failed')]);
        }
    }
    ...
    public function get_award_points($id = null)
    {
        ...
        $this->rerp->send_json(['sa_points' => $row->award_points]);
    }
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $packaging_link    = anchor('admin/sales/packaging/$1', '<i class="fa fa-archive"></i> ' . lang('packaging'), 'data-toggle="modal" data-target="#myModal"');
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, COALESCE({$this->db->dbprefix('deliveries')}.status, 'Not Set') as delivery_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
    ...
    public function gift_card_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    $this->rerp->checkPermissions('delete_gift_card');
                    foreach ($_POST['val'] as $id) {
    ...
    public function modal_view($id = null)
    {
        ...
        $inv                 = $this->sales_model->getInvoiceByID($id);
        if (!$this->session->userdata('view_right')) {
            $this->rerp->view_rights($inv->created_by, true);
        }
        $this->data['customer']    = $this->site->getCompanyByID($inv->customer_id);
        $this->data['biller']      = $this->site->getCompanyByID($inv->biller_id);
	    ...
	    if ( ( $delivery_schedule = Erp_Delivery_Schedule::get_delivery_schedule_by_sales_id( $inv->id ) ) && ( $inv->shop || $inv->api ) ){
    ...
    public function pdf( $id = null, $view = null, $save_buffer = null ) {
        $this->rerp->checkPermissions();
        ...
        $pdf_template = 'sales/pdf';
	    $pdf_template_file = $this->getCurrentThemeViews( $pdf_template, false, false );
	    ...
	    if ( isset( $this->themeSettings['theme_options']['pdf_settings']['template_type']['value'] ) ) {
		    ...
		    $_pdf_template_file = $this->getCurrentThemeViews( 'pages/pdf_invoice_' . $_pdf_template . '.php', true );
		    if ( 'default' != $_pdf_template && file_exists( $_pdf_template_file ) ) {
			    $pdf_template_file = $this->getCurrentThemeViews( 'pages/pdf_invoice_' . $_pdf_template, false, true );
		    }
	    }
	    ...
	    if ( $view ) {
		    $this->load->view( $pdf_template_file, $this->data );
	    } elseif ( $save_buffer ) {
		    return $this->rerp->generate_pdf( $html, $name, $save_buffer, $this->data['biller']->invoice_footer );
	    } else {
		    $this->rerp->generate_pdf( $html, $name, false, $this->data['biller']->invoice_footer );
	    }
    }

    public function pdf_delivery($id = null, $view = null, $save_buffer = null)
    {
        $this->rerp->checkPermissions();
        ...
        if ($view) {
            ...
            return $this->rerp->generate_pdf($html, $name, $save_buffer);
        } else {
            $this->rerp->generate_pdf($html, $name);
        }
    }
    ...
    public function return_sale( $id = null, int $shipment_id = null )
    {
        $this->rerp->checkPermissions('return_sales');
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    // $unit_price = $real_unit_price;
                    $pr_discount      = $this->site->calculateDiscount($item_discount, $unit_price);
                    $unit_price       = $this->rerp->formatDecimal(($unit_price - $pr_discount), 4);
                    $item_net_price   = $unit_price;
                    $pr_item_discount = $this->rerp->formatDecimal($pr_discount * $item_unit_quantity, 4);
                    $product_discount += $pr_item_discount;
                    $pr_item_tax = $item_tax = 0;
                    $tax         = '';
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = $this->rerp->formatDecimal((($item_net_price * $item_unit_quantity) + $pr_item_tax), 4);
            ...
            $order_tax      = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
            $total_tax      = $this->rerp->formatDecimal(($product_tax + $order_tax), 4);
            $grand_total    = $this->rerp->formatDecimal(($this->rerp->formatDecimal($total) + $this->rerp->formatDecimal($total_tax) + $this->rerp->formatDecimal($return_surcharge) + (0 - $shipping) - $this->rerp->formatDecimal($order_discount)), 4);
            $data           = [
                'date'              => $date,
                'sale_id'           => $id,
                ...
                'order_tax'         => $order_tax,
                'total_tax'         => $total_tax,
                'surcharge'         => $this->rerp->formatDecimal($return_surcharge),
                'grand_total'       => $grand_total,
                'created_by'        => $this->session->userdata('user_id'),
	        ...
	        $return_data           = [
		        ...
		        'grand_total'       => $grand_total,
		        'total_items'       => 0,
		        'paid'              => (0 - $this->input->post('amount-paid')),
		        'created_by'        => $this->session->userdata('user_id'),
		        'hash'              => hash('sha256', microtime() . mt_rand()),
	        ];
            ...
            // $this->rerp->print_arrays($data, $products, $si_return, $payment);
        }
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, $si_return) ) {
            ...
            if( is_int($shipment_id) ) {
	            $inv_items = $this->delivery_model->getAllInvoiceItemsForShipment( $shipment_id );
            } else {
	            $inv_items = $this->sales_model->getAllInvoiceItems($id);
            }
            ...
            foreach ($inv_items as $item) {
                ...
                $row->item_tax        = $item->item_tax      > 0 ? $item->item_tax      / $item->quantity : 0;
                $row->item_discount   = $item->item_discount > 0 ? $item->item_discount / $item->quantity : 0;
                $row->price           = $this->rerp->formatDecimal($item->net_unit_price + $this->rerp->formatDecimal($item->item_discount / $item->unit_quantity));
                $row->unit_price      = $row->tax_method ? $item->unit_price + $this->rerp->formatDecimal($item->item_discount / $item->unit_quantity) - $this->rerp->formatDecimal($item->item_tax / $item->unit_quantity) : $item->unit_price + ($item->item_discount / $item->unit_quantity);
                $row->real_unit_price = $item->real_unit_price;
                $row->base_quantity   = $item->quantity;
                $row->base_unit       = isset($row->unit) ? $row->unit : $item->product_unit_id;
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        $sale = $this->sales_model->getInvoiceByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->rerp->hrld($sale->date));
                ...
                foreach ($final as $csv_pr) {
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_price']) && isset($csv_pr['quantity'])) {
                        if ($product_details = $this->sales_model->getProductByCode($csv_pr['code'])) {
                            ...
                            $item_net_price = $this->rerp->formatDecimal($csv_pr['net_unit_price']);
                            ...
                            if (isset($item_code) && isset($item_net_price) && isset($item_quantity)) {
                                ...
                                $item_net_price   = $this->rerp->formatDecimal(($item_net_price - $pr_discount), 4);
                                $pr_item_discount = $this->rerp->formatDecimal(($pr_discount * $item_quantity), 4);
                                $product_discount += $pr_item_discount;
                                ...
                                if ($tax_details) {
                                    ...
                                    $item_tax = $this->rerp->formatDecimal($ctax['amount']);
                                    $tax      = $ctax['tax'];
                                    // $this->rerp->print_arrays($product_details);
                                    // if (!$product_details || (!empty($product_details) && $product_details->tax_method != 1)) {
                                    $unit_price = $unit_price + $item_tax;
                                    // }
                                    $pr_item_tax = $this->rerp->formatDecimal($item_tax * $item_quantity, 4);
                                    if ($this->Settings->indian_gst && $gst_data = $this->gst->calculteIndianGST($pr_item_tax, ($biller_details->state == $customer_details->state), $tax_details)) {
                                        $total_cgst += $gst_data['cgst'];
                                        $total_sgst += $gst_data['sgst'];
                                        $total_igst += $gst_data['igst'];
                                ...
                                $product_tax += $pr_item_tax;
                                $subtotal = $this->rerp->formatDecimal(($unit_price * $item_quantity), 4);
                                ...
                                $product = [
                                    ...
                                    'subtotal'          => $subtotal,
                                    'serial_no'         => $item_serial,
                                    'unit_price'        => $this->rerp->formatDecimal($unit_price, 4),
                                    'real_unit_price'   => $this->rerp->formatDecimal(($unit_price + $pr_discount), 4),
                                ];

                                $products[] = ($product + $gst_data);
                                $total += $this->rerp->formatDecimal(($item_net_price * $item_quantity), 4);
                            }
                        } else {
    ...
    {
        $this->rerp->checkPermissions('gift_cards', true);
        $error  = null;
        ...
        $data             = ['card_no' => $gcData[0],
            ...
            'customer'                 => $customer,
            'balance'                  => $gcData[1],
            'expiry'                   => (!empty($gcData[3])) ? $this->rerp->fsd($gcData[3]) : null,
            'created_by'               => $this->session->userdata('user_id'),
        ];

        if (!$error) {
            if ($this->sales_model->addGiftCard($data)) {
                $this->rerp->send_json(['result' => 'success', 'message' => lang('gift_card_added')]);
            }
        } else {
            $this->rerp->send_json(['result' => 'failed', 'message' => $error]);
        }
    }
    ...
    {
        $this->rerp->checkPermissions('add_gift_card', true);
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->input->post('expiry')) {
                $card_data['expiry'] = $this->rerp->fld(trim($this->input->post('expiry')));
            }
        ...
        if ($gc = $this->site->getGiftCardByNO($no)) {
            if ($gc->expiry) {
                if ($gc->expiry >= date('Y-m-d')) {
                    $this->rerp->send_json($gc);
                } else {
                    $this->rerp->send_json(false);
                }
            } else {
                $this->rerp->send_json($gc);
            }
    ...
    public function view($id = null)
    {
	    ...
	    $this->data['paypal']       = $this->sales_model->getPaypalSettings();
	    $this->data['skrill']       = $this->sales_model->getSkrillSettings();
	    $this->data['packed_item'] = $this->sales_model->getAllPackedItemsIDBySaleID( $inv->id );
	    $this->data['unpacked_item'] = $this->sales_model->getUnpackedItemsBySaleID( $inv->id );
