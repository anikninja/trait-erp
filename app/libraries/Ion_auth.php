...
class Ion_auth
{
    ...
    public function __construct()
    {
        ...
        if (!$this->logged_in() && get_cookie('identity') && get_cookie('remember_code')) {
            if ($this->auth_model->login_remembered_user()) {
            	$requestedPage = $this->session->userdata('requested_page');
            	if ( $requestedPage ) {
		            $this->session->unset_userdata('requested_page');
	            }
	            redirect( $requestedPage ? $requestedPage : ( isset( $_SERVER['HTTP_REFERER'] ) ? $_SERVER['HTTP_REFERER'] : 'admin/welcome' ) );
            }
        }
        ...
        if ($this->auth_model->forgotten_password($identity)) {   //changed
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $msg     = file_get_contents('./themes/' . $this->Settings->adminTheme . 'email_templates/forgot_password.html');
                    ...
                    try {
                        if ($this->rerp->send_email($user->email, $subject, $message)) {
                            $this->set_message('forgot_password_successful');
                            return true;
                        } else {
    ...
    public function forgotten_password_complete($code)
    {
        ...
        if ($new_password) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $msg     = file_get_contents('./themes/' . $this->Settings->adminTheme . 'email_templates/new_password.html');
                $message = $this->parser->parse_string($msg, $parse_data);
                $subject = lang('email_new_password_subject') . ' - ' . $this->Settings->site_name;

                try {
                    if ($this->rerp->send_email($profile->email, $subject, $message)) {
                        $this->set_message('password_change_successful');
                        $this->auth_model->trigger_events(['post_password_change', 'password_change_successful']);
                        return true;
                    } else {
	    ...
	    if ( ! $email_activation || $active ) {
		    ...
		    if ( $id !== false ) {
			    if ( $notify ) {
				    ...
				    $msg = file_get_contents( './themes/' . $this->Settings->adminTheme . 'email_templates/credentials.html' );
				    $message = $this->parser->parse_string( $msg, $parse_data );
				    $subject = $this->lang->line( 'new_user_created' ) . ' - ' . $this->Settings->site_name;
				    try {
					    $this->rerp->send_email( $email, $subject, $message );
				    } catch ( Exception $e ) {
					    $this->set_error( $e->getMessage() );
				    }
			    }
			    ...
			    $this->auth_model->trigger_events( [
		    ...
		    if ( ! $this->deactivate_user_account( $id ) ) {
		    	return false;
		    }

		    $activation_email = $this->send_activation_email( $id, $email, $additional_data );
		    if ( $activation_email || is_array( $activation_email ) ) {
		    	return $id;
		    }
    ...
    public function send_activation_email( $id, $email, $additional_data ) {
	    ...
	    if ( ! $this->config->item( 'use_ci_email', 'ion_auth' ) ) {
		    ...
		    $msg     = file_get_contents('./themes/' . $this->Settings->adminTheme . 'email_templates/activate_email.html');
		    $message = $this->parser->parse_string($msg, $parse_data);
		    $subject = $this->lang->line('email_activation_subject') . ' - ' . $this->Settings->site_name;
		
		    try {
			    if ( $this->rerp->send_email( $email, $subject, $message ) ) {
				    $this->auth_model->trigger_events( [ 'post_account_creation', 'post_account_creation_successful', 'activation_email_successful', ] );
				    $this->set_message( 'activation_email_successful' );
