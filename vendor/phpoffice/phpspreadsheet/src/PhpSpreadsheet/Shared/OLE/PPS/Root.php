...
class Root extends PPS
{
    ...
    public function save($filename)
    {
        ...
        $this->rerpllBlockSize = pow(
            2,
            (isset($this->rerpllBlockSize)) ? self::adjust2($this->rerpllBlockSize) : 6
        );
    ...
    public function _calcSize(&$raList)
    {
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            if ($raList[$i]->Type == OLE::OLE_PPS_TYPE_FILE) {
                ...
                if ($raList[$i]->Size < OLE::OLE_DATA_SIZE_SMALL) {
                    $iSBcnt += floor($raList[$i]->Size / $this->rerpllBlockSize)
                                  + (($raList[$i]->Size % $this->rerpllBlockSize) ? 1 : 0);
                } else {
        ...
        $iSmallLen = $iSBcnt * $this->rerpllBlockSize;
    ...
    public function _makeSmallData(&$raList)
    {
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            ...
            if ($raList[$i]->Type == OLE::OLE_PPS_TYPE_FILE) {
                ...
                if ($raList[$i]->Size < OLE::OLE_DATA_SIZE_SMALL) {
                    $iSmbCnt = floor($raList[$i]->Size / $this->rerpllBlockSize)
                                  + (($raList[$i]->Size % $this->rerpllBlockSize) ? 1 : 0);
                    // Add to SBD
                    $jB = $iSmbCnt - 1;
                    for ($j = 0; $j < $jB; ++$j) {
                    ...
                    $sRes .= $raList[$i]->_data;
                    if ($raList[$i]->Size % $this->rerpllBlockSize) {
                        $sRes .= str_repeat("\x00", $this->rerpllBlockSize - ($raList[$i]->Size % $this->rerpllBlockSize));
                    }
                    // Set for PPS
