...
if ( ! function_exists( 'cs_lazy_image' ) ) {
	function cs_lazy_image( $image, $attrs = null, $placeholder = null ) {
		$attrs = ci_parse_args( $attrs, [ 'class' => '', 'lazy' => '', 'alt' => '' ] );
		if ( ! $placeholder ) {
		...
		if ( 'echo' !== $attrs['lazy'] ) {
			$_attrs['data-echo'] = $image;
		}
		if ( false !== strpos( $attrs['class'], 'owl2' ) ) {
			$_attrs['data-src-retina'] = $image;
		}
		$img = '<img ';
	...
	function build_category_tree( $list, $selected = [], $classes = [], $url_path = '', $dept = 0 ) {
		...
		foreach ( $list as $idx => $item ) {
			...
			if ( isset( $item->has_children ) && $item->has_children ) {
				$output .= sprintf(
					'<li class="%s"><a href="%s">%s</a><span class="hidden"><i class="fa fa-chevron-right"></i></span>%s</li>',
					$liClasses,
					$url_path . '/' . $item->slug,
					$item->name,
