                    ...
                    <div id="leftdiv">
                        <div id="printhead">
                            ...
                            <?php
                                ...
                                echo $this->lang->line('date') . ' ' . $this->rerp->hrld(date('Y-m-d H:i:s'));
                            ?>
                        </div>
                        <div id="left-top">
                            <div
...
<div id="brands-slider">
    <div id="brands-list">
        ...
        foreach ( $brands as $brand ) {
	            $image = get_image_url_thumb_first( $brand->image, $Settings->noImage );
	
	            echo sprintf(
		            '<button id="brand-%s" type="button" value="%s" class="btn-prni brand"><img src="%s" class="img-rounded img-thumbnail"><span>%s</span></button>',
		            $brand->id,
		            $brand->id,
		            $image,
		            $brand->name
...
<div id="category-slider">
    ...
    <div id="category-list">
        ...
        foreach ( $categories as $category ) {
            	$image = get_image_url_thumb_first( $category->image, $Settings->noImage );
                echo sprintf( '<button id="category-%s" type="button" value="%s" class="btn-prni category"><img src="%s" class="img-rounded img-thumbnail"><span>%s</span></button>',
	                $category->id,
	                $category->id,
	                $image,
	                $category->name
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <?php echo form_input('gcexpiry', $this->rerp->hrsd(date('Y-m-d', strtotime('+2 year'))), 'class="form-control date" id="gcexpiry"'); ?>
                </div>
...
<?php if ($this->session->userdata('remove_posls')) {
        ...
        <?php $this->rerp->unset_data('remove_posls');
    }
        ?>
        widthFunctions();
        <?php if ($suspend_sale) {
...
<?php if ($this->input->get('customer')) {
        ...
        if (!localStorage.getItem('positems')) {
            ...
            localStorage.setItem('poscustomer', <?= $customer ? $customer->id : ""; ?>);
        }
        <?php
        } else {
            ?>
    ...
    <?php if ($pos_settings->remote_printing == 1) {
            ?>
    function Popup(data) {
        var mywindow = window.open('', 'rerp_pos_print', 'height=500,width=300');
