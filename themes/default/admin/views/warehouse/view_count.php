...
<div class="modal-dialog">
                ...
                <table class="table table-bordered table-condensed">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?= $this->rerp->hrld($stock_count->date); ?></td>
                            </tr>
                            <tr>
                                <td><?= lang('end_date'); ?></td>
                                <td><?= $this->rerp->hrld($stock_count->updated_at); ?></td>
                            </tr>
                            <tr>
    ...
    ?>
        ...
        foreach ($stock_count_items as $row): ?>
                                <tr>
                                    ...
                                    <td style="text-align:center; width:80px;">
                                        <?= $this->rerp->formatQuantity($row->expected); ?>
                                    </td>
                                    <td style="text-align:center; width:80px;">
                                        <?= $this->rerp->formatQuantity($row->counted); ?>
                                    </td>
                                    <td style="text-align:right; width:80px;">
                                        <?= $this->rerp->formatQuantity($row->counted - $row->expected); ?>
                                    </td>
                                    <td style="text-align:right; width:100px;">
                                        <?= $this->rerp->formatMoney($row->cost * ($row->counted - $row->expected)); ?>
                                    </td>
                                </tr>
                                <?php $r++;
        ...
        endforeach; ?>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <?= $this->rerp->formatQuantity($items); ?>
                                    </th>
                                    <th style="text-align:right; width:100px;">
                                        <?= $this->rerp->formatMoney($total); ?>
                                    </th>
                                </tr>
                            </tfoot>
                        </table>
        ...
        echo '<div class="well well-sm">' . lang('no_mismatch_found') . '</div>';
                    ...
                    <?php
                    if ($adjustment) {
                        echo '<a href="' . admin_url('warehouse/view_adjustment/' . $adjustment->id) . '" class="btn btn-primary btn-block no-print" data-toggle="modal" data-target="#myModal2">' . lang('view_adjustment') . '</a>';
                    } elseif (!empty($stock_count_items)) {
                        echo '<a href="' . admin_url('warehouse/add_adjustment/' . $stock_count->id) . '" class="btn btn-primary btn-block no-print">' . lang('add_adjustment') . '</a>';
                    }
                    ?>
                </div>
            </div>
