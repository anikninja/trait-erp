    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            <tr>
                ...
                <td width="70%"><?php echo $this->rerp->hrld($delivery->date); ?></td>
            </tr>
            <tr>
            ...
            <?php if ( $delivery->note ) { ?>
                <tr>
                    ...
                    <td><?php echo $this->rerp->decode_html($delivery->note); ?></td>
                </tr>
            <?php } ?>
            </tbody>
        ...
        <table class="table table-bordered table-hover table-striped">
            ...
            foreach ( $rows as $row ): ?>
                <tr>
                    ...
                    <td style="vertical-align:middle;"><?= $row->product_code . ' - ' . $row->product_name . ($row->variant ? ' (' . $row->variant . ')' : '');
                        if ( $row->details ) {
                            echo '<br><strong>' . lang('product_details') . '</strong> ' . html_entity_decode( $row->details );
                        }
                    ?></td>
                    <td style="width: 150px; text-align:center; vertical-align:middle;"><?= $this->rerp->formatQuantity( $row->unit_quantity ) . ' ' . $row->product_unit_code; ?></td>
                </tr>
                <?php
                $r++;
            endforeach;
