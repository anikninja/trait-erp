...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        <?php if ($inv) {
    ?>
        //localStorage.setItem('redate', '<?= $this->rerp->hrld($inv->date) ?>');
        localStorage.setItem('reref', '<?= $reference ?>');
        localStorage.setItem('renote', '<?= $this->rerp->decode_html($inv->note); ?>');
                    ...
                    ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('order_discount', ($_POST['rediscount'] ?? $inv->order_discount > 0 ? $this->rerp->formatDecimal($inv->order_discount_id) : ''), 'class="form-control input-tip" id="rediscount"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang('shipping', 'shipping'); ?>
                                <?php echo form_input('shipping', ($_POST['shipping'] ?? $inv->shipping > 0 ? $this->rerp->formatDecimal($inv->shipping) : ''), 'class="form-control input-tip" id="shipping" required="required"'); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            ...
                            if ($inv->payment_status == 'paid') {
                                echo '<div class="alert alert-success">' . lang('payment_status') . ': <strong>' . $inv->payment_status . '</strong> & ' . lang('paid_amount') . ' <strong>' . $this->rerp->formatMoney($inv->paid) . '</strong></div>';
                            } else {
                                echo '<div class="alert alert-warning">' . lang('payment_status_not_paid') . ' ' . lang('payment_status') . ': <strong>' . $inv->payment_status . '</strong> & ' . lang('paid_amount') . ' <strong>' . $this->rerp->formatMoney($inv->paid) . '</strong></div>';
                            }
                            ?>
                        </div>
