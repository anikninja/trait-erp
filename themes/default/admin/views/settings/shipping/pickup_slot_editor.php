...
$action = 'system_settings/pickup_slot_editor/'  . ( $edit ? $slot->getId() : $area->getId() . '/add' );
echo admin_form_open( $action, $attrib );
echo form_hidden( 'area_id', $area->getId() );
?>
<div class="box">
	<div class="box-header">
		<h2 class="blue"><i class="fa-fw fa fa-<?= $edit ? 'edit' : 'plus'; ?>"></i><?php printf( $edit ? lang( 'edit_x' ) : lang('add_x'), sprintf( lang('pickup_x'), lang( 'slot' ) ) ); ?></h2>
	</div>
