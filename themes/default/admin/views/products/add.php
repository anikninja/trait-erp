...
$base_units      = isset( $base_units ) ? $base_units : [];
$tax_rates       = isset( $tax_rates ) ? $tax_rates : [];
$product_images  = isset( $product_images ) ? $product_images : [];
$product_options = isset( $product_options ) ? $product_options : [];
$custom_fields   = isset( $custom_fields ) ? $custom_fields : [];
$custom_values   = isset( $custom_values ) ? $custom_values : [];
$combo_items     = isset( $combo_items ) ? $combo_items : [];
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        */
        ...
        <?php
	    if( isset($_POST['cash_back_percentage_on']) ){
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
                        <?= lang('brand', 'brand') ?>
                        <?= form_dropdown('brand', $brands, ($_POST['brand'] ?? ($product ? $product->brand : '')), 'class="form-control select" id="brand" placeholder="' . lang('select') . ' ' . lang('brand') . '" style="width:100%"'); ?>
                    </div>
                    <div class="form-group all">
                    ...
                    <div class="form-group standard">
                        ...
                        <?= form_input('cost', ($_POST['cost'] ?? ($product ? $this->rerp->formatDecimal($product->cost) : '')), 'class="form-control tip" id="cost" required="required"') ?>
                    </div>
                    <div class="form-group all">
                        <?= lang('product_price', 'price') ?>
                        <?= form_input('price', ($_POST['price'] ?? ($product ? $this->rerp->formatDecimal($product->price) : '')), 'class="form-control tip" id="price" required="required"') ?>
                    </div>
	                ...
	                <div id="cash_back_settings" style="display:none;">
		                <div class="well well-sm">
                            <div class="form-group">
                            ...
                            <div class="form-group" id="cash_back_percentage_field" >
				                ...
				                <?= form_input('cash_back_percentage', set_value('cash_back_percentage'), 'class="form-control tip" id="cash_back_percentage"'); ?>
                            </div>
			                <div class="form-group" id="cash_back_amount_field">
				                <?= lang('cash_back_amount', 'cash_back_amount'); ?>
				                <?= form_input('cash_back_amount', set_value('cash_back_amount' ), 'class="form-control tip" id="cash_back_amount"'); ?>
			                </div>
			                <div class="form-group">
				                <?= lang('start_date', 'cash_back_start_date'); ?>
				                <?= form_input('cash_back_start_date', set_value('cash_back_start_date' ), 'class="form-control tip date" id="cash_back_start_date"'); ?>
			                </div>
			                <div class="form-group">
				                <?= lang('end_date', 'cash_back_end_date'); ?>
				                <?= form_input('cash_back_end_date', set_value('cash_back_end_date' ), 'class="form-control tip date" id="cash_back_end_date"'); ?>
			                </div>
		                </div>
	                </div>
                    ...
                    <?php if ($Settings->tax1) {
                        ...
                        <div class="form-group all">
                            ...
                            <?= form_dropdown('tax_rate', $tax_rates, ($_POST['tax_rate'] ?? ($product ? $product->tax_rate : $Settings->default_tax_rate)), 'class="form-control select" id="tax_rate" placeholder="' . lang('select') . ' ' . lang('product_tax') . '" style="width:100%"'); ?>
                        </div>
                        <div class="form-group all">
                    ...
                    <div class="form-group standard">
                        ...
                        <div
                            class="input-group"> <?= form_input('alert_quantity', ($_POST['alert_quantity'] ?? ($product ? $this->rerp->formatQuantityDecimal($product->alert_quantity) : '')), 'class="form-control tip" id="alert_quantity"') ?>
                ...
                <div class="col-md-6 col-md-offset-1">
                    <div class="standard">
                        ...
                        <div class="form-group">
	                        ...
	                        <label for="attributes" class="padding05"><?= lang('product_has_attributes'); ?></label>
                                ...
                                <table id="attrTable" class="table table-bordered table-condensed table-striped" style="<?= $this->input->post('attributes') || $product_options ? '' : 'display:none;'; ?>margin-bottom: 0; margin-top: 10px;">
                                    ...
                                    if ( $this->input->post( 'attributes' ) ) {
                                        ...
                                        foreach ($product_options as $option) {
                                            echo '<tr class="attr">
												<td>
													<input type="hidden" name="attr_name[]" value="' . $option->name . '">
													<span>' . $option->name . '</span>
												</td>
												<td class="code text-center">
													<input type="hidden" name="attr_warehouse[]" value="' . $option->warehouse_id . '">
													<input type="hidden" name="attr_wh_name[]" value="' . $option->wh_name . '">
													<span>' . $option->wh_name . '</span>
												</td>
												<td class="quantity text-center">
													<input type="hidden" name="attr_quantity[]" value="' . $this->rerp->formatQuantityDecimal($option->wh_qty) . '">
													<span>' . $this->rerp->formatQuantity($option->wh_qty) . '</span>
												</td>
												<td class="price text-right">
													<input type="hidden" name="attr_price[]" value="' . $this->rerp->formatMoney($option->price) . '">
													<span>' . $this->rerp->formatMoney($option->price) . '</span>
													</td>
												<td class="text-center"><i class="fa fa-times delAttr"></i></td>
											</tr>';
                                        }
                                ...
                                if ($product) {
                                    ...
                                    if (!empty($warehouses_products)) {
                                        ...
                                        foreach ($warehouses_products as $wh_pr) {
                                            echo '<span class="bold text-info">' . $wh_pr->name . ': <span class="padding05" id="rwh_qty_' . $wh_pr->id . '">' . $this->rerp->formatQuantity($wh_pr->quantity) . '</span>' . ($wh_pr->rack ? ' (<span class="padding05" id="rrack_' . $wh_pr->id . '">' . $wh_pr->rack . '</span>)' : '') . '</span><br>';
                                        }
                                        echo '</div>';
                                    }
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group all">
                        ...
                        <?= form_textarea('details', ($_POST['details'] ?? ($product ? $product->details : '')), 'class="form-control" id="product_details_for_invoice"'); ?>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
	    ...
	    <?= isset($_POST['cash_back']) ? '$("#cash_back").iCheck("check");' : '' ?>
	    <?= isset($_POST['cash_back_percentage_on']) ? '$("#cash_back_percentage_on").iCheck("check");' : '' ?>
            ...
            for (var i in attrs) {
                if (attrs[i] !== '') {
	                ...
	                __ATTRS__ = `<tr class="attr">
					   ...
					   <td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="<?= $warehouse->id; ?>"><span><?= $warehouse->name; ?></span></td>
    ...
    $(document).ready(function () {
        ...
        $('#supplier_price').val("<?= $product->supplier1price == 0 ? '' : $this->rerp->formatDecimal($product->supplier1price); ?>");
        <?php
            } ?>
        ...
        $('#supplier_2_price').val("<?= $product->supplier2price == 0 ? '' : $this->rerp->formatDecimal($product->supplier2price); ?>");
        <?php
            } ?>
        ...
        $('#supplier_3_price').val("<?= $product->supplier3price == 0 ? '' : $this->rerp->formatDecimal($product->supplier3price); ?>");
        <?php
            } ?>
        ...
        $('#supplier_4_price').val("<?= $product->supplier4price == 0 ? '' : $this->rerp->formatDecimal($product->supplier4price); ?>");
        <?php
            } ?>
        ...
        $('#supplier_5_price').val("<?= $product->supplier5price == 0 ? '' : $this->rerp->formatDecimal($product->supplier5price); ?>");
        <?php
            } ?>
