                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="row">
                            <div class="col-sm-5">
	                            <?php
	                            $image = get_image_url( $product->image, false, true );
	                            if ( ! $image ) {
		                            $image = $Settings->noImage;
	                            }
	                            ?>
	                            <img id="pr-image" src="<?= $image; ?>" alt="<?= $product->name ?>" class="img-responsive img-thumbnail"/>

                                <div id="multiimages" class="padding10">
                                    <?php
                                    $style = "max-width:{$Settings->twidth}px; max-height:{$Settings->theight}px;";
                                    if ( ! empty( $images ) ) {
								        echo '<a class="img-thumbnail" data-toggle="lightbox" data-gallery="multiimages" data-parent="#multiimages" href="' . $image . '" style="margin-right:5px;">' .
								             '<img class="img-responsive" src="' . $image . '" alt="' . $product->image . '" style="' . $style . '">' .
								             '</a>';
	                                    foreach ( $images as $ph ) {
		                                    $photo = get_image_url( $ph->photo, false, true );
		                                    if ( ! $photo ) {
		                                    	continue;
		                                    }
		                                    $thumb = get_image_url_thumb_first( $ph->photo, $Settings->noImage );
								            echo '<div class="gallery-image">' .
								                 '<a class="img-thumbnail" data-toggle="lightbox" data-gallery="multiimages" data-parent="#multiimages" href="' . $photo . '" style="margin-right:5px;">' .
								                    '<img class="img-responsive" src="' . $thumb . '" alt="' . $ph->photo . '" style="' . $style . '">' .
								                 '</a>';
                                    ...
                                    <table class="table table-borderless table-striped dfTable table-right-left">
                                        ...
                                        <tr>
                                            ...
                                            <img src="<?= admin_url('misc/barcode/' . $product->code . '/' . $product->barcode_symbology . '/74/0'); ?>" alt="<?= $product->code; ?>" class="bcimg" />
                                                <?= $this->rerp->qrcode('link', urlencode(admin_url('products/view/' . $product->id)), 2); ?>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><?= lang('type'); ?></td>
                                        ...
                                        <?php if ($Owner || $Admin) {
                                        echo '<tr><td>' . lang('cost') . '</td><td>' . $this->rerp->formatMoney($product->cost) . '</td></tr>';
                                        echo '<tr><td>' . lang('price') . '</td><td>' . $this->rerp->formatMoney($product->price) . '</td></tr>';
                                        if ($product->promotion) {
                                            echo '<tr><td>' . lang('promotion') . '</td><td>' . $this->rerp->formatMoney($product->promo_price) . ' (' . $this->rerp->hrsd($product->start_date) . ' - ' . $this->rerp->hrsd($product->end_date) . ')</td></tr>';
                                        }
                                    } else {
                                        ...
                                        if ($this->session->userdata('show_price')) {
                                            echo '<tr><td>' . lang('price') . '</td><td>' . $this->rerp->formatMoney($product->price) . '</td></tr>';
                                            if ($product->promotion) {
                                                echo '<tr><td>' . lang('promotion') . '</td><td>' . $this->rerp->formatMoney($product->promo_price) . ' (' . $this->rerp->hrsd($product->start_date) . ' - ' . $this->rerp->hrsd($product->start_date) . ')</td></tr>';
                                            }
                                        }
                                    }
                                        ?>
                                        ...
                                        <?php if ($product->alert_quantity != 0) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo $this->rerp->formatQuantity($product->alert_quantity); ?></td>
                                            </tr>
                                        <?php
                                        } ?>
                                        <?php if ($variants) {
                                        ...
                                        <?php if ((!$Supplier || !$Customer) && !empty($warehouses) && $product->type == 'standard') {
                                            ...
                                            <div class="table-responsive">
                                                ...
                                                if ($warehouse->quantity != 0) {
                                                    echo '<tr><td>' . $warehouse->name . ' (' . $warehouse->code . ')</td><td><strong>' . $this->rerp->formatQuantity($warehouse->quantity) . '</strong>' . ($warehouse->rack ? ' (' . $warehouse->rack . ')' : '') . '</td></tr>';
                                                }
                                            } ?>
                                                    </tbody>
                                                </table>
                                    ...
                                    <div class="col-sm-7">
                                        <?php if ($product->type == 'combo') {
                                            ...
                                            <div class="table-responsive">
                                                ...
                                                echo '<tr><td>' . $combo_item->name . ' (' . $combo_item->code . ') </td><td>' . $this->rerp->formatQuantity($combo_item->qty) . '</td></tr>';
                                            } ?>
                                                    </tbody>
                                                </table>
                                            </div>
                                            ...
                                            <div class="table-responsive">
                                                ...
                                                echo '<th>' . lang('price') . '</th>';
                                                    ...
                                                    foreach ($options as $option) {
                                                        if ($option->wh_qty != 0) {
                                                            echo '<tr><td>' . $option->wh_name . '</td><td>' . $option->name . '</td><td class="text-center">' . $this->rerp->formatQuantity($option->wh_qty) . '</td>';
                                                            if ($Owner || $Admin && (!$Customer || $this->session->userdata('show_cost'))) {
                                                                echo '<td class="text-right">' . $this->rerp->formatMoney($option->cost) . '</td><td class="text-right">' . $this->rerp->formatMoney($option->price) . '</td>';
                                                            }
                                                            echo '</tr>';
                                                        }
                                                    } ?>
