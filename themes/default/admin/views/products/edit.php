...
<script type="text/javascript">
    $(document).ready(function () {
	    ...
	    <?php
            if( isset($_POST['cash_back_percentage_on']) || $product->cash_back_percentage > 0 ){
	            echo "$('#cash_back_percentage_field').show();";
	            echo "$('#cash_back_amount_field').hide();";
            }
            else{
	            echo "$('#cash_back_amount_field').show();";
	            echo "$('#cash_back_percentage_field').hide();";
            }
        ?>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
                        <?= lang('brand', 'brand') ?>
                        <?= form_dropdown('brand', $brands, (isset($_POST['brand']) ? $_POST['brand'] : ($product ? $product->brand : '')), 'class="form-control select" id="brand" placeholder="' . lang('select') . ' ' . lang('brand') . '" style="width:100%"'); ?>
                    </div>
	                <div class="form-group all">
		                <?= lang('category', 'category') ?>
		                <select name="category" class="form-control select" id="category" placeholder="<?= sprintf( lang( 'select_x' ), lang( 'category' ) ); ?>" required="required" style="width:100%">
			                <option value=""></option>
			                <?= $categories; ?>
		                </select>
	                </div>
                    ...
                    <div class="form-group standard">
                        ...
                        <?= form_input('cost', (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->rerp->formatDecimal($product->cost) : '')), 'class="form-control tip" id="cost" required="required"') ?>
                    </div>
                    <div class="form-group all">
                        <?= lang('product_price', 'price') ?>
                        <?= form_input('price', (isset($_POST['price']) ? $_POST['price'] : ($product ? $this->rerp->formatDecimal($product->price) : '')), 'class="form-control tip" id="price" required="required"') ?>
                    </div>

                    <div class="form-group">
                        <input type="checkbox" class="checkbox" value="1" name="promotion" id="promotion" <?= $this->input->post('promotion') ? 'checked="checked"' : ''; ?>>
                        <label for="promotion" class="padding05"><?= lang('promotion'); ?></label>
                    </div>
                    ...
                    <div id="promo"<?= $product->promotion ? '' : ' style="display:none;"'; ?>>
                        <div class="well well-sm">
                            <div class="form-group">
                                ...
                                <?= form_input('promo_price', set_value('promo_price', $product->promo_price ? $this->rerp->formatDecimal($product->promo_price) : ''), 'class="form-control tip" id="promo_price"'); ?>
                            </div>
                            <div class="form-group">
                                <?= lang('start_date', 'start_date'); ?>
                                <?= form_input('start_date', set_value('start_date', $product->start_date ? $this->rerp->hrsd($product->start_date) : ''), 'class="form-control tip date" id="start_date"'); ?>
                            </div>
                            <div class="form-group">
                                <?= lang('end_date', 'end_date'); ?>
                                <?= form_input('end_date', set_value('end_date', $product->end_date ? $this->rerp->hrsd($product->end_date) : ''), 'class="form-control tip date" id="end_date"'); ?>
                            </div>
                        </div>
                    </div>
	
	                <div class="form-group">
		                <input type="checkbox" class="checkbox" value="1" name="cash_back" id="cash_back" <?= $this->input->post('cash_back') ? 'checked="checked"' : ''; ?>>
		                <label for="cash_back" class="padding05"><?= lang('cash_back'); ?></label>
	                </div>
	
	                <div id="cash_back_settings"<?= $product->cash_back ? '' : ' style="display:none;"'; ?>>
		                <div class="well well-sm">
                            <div class="form-group">
                                <input type="checkbox" class="checkbox" value="1" name="cash_back_percentage_on" id="cash_back_percentage_on" <?= $this->input->post('cash_back_percentage_on') ? 'checked="checked"' : ''; ?>>
                                <label for="cash_back_percentage_on" class="padding05"><?= lang('set_cash_back_percentage'); ?></label>
                            </div>
                            <div class="form-group" id="cash_back_percentage_field" >
				                <?= lang('cash_back_percentage', 'cash_back_percentage'); ?>
				                <?= form_input('cash_back_percentage', set_value('cash_back_percentage', $product->cash_back_percentage ? $this->rerp->formatDecimal($product->cash_back_percentage) : ''), 'class="form-control tip" id="cash_back_percentage"'); ?>
                            </div>
			                <div class="form-group" id="cash_back_amount_field">
				                <?= lang('cash_back_amount', 'cash_back_amount'); ?>
				                <?= form_input('cash_back_amount', set_value('cash_back_amount', $product->cash_back_amount ? $this->rerp->formatDecimal($product->cash_back_amount) : ''), 'class="form-control tip" id="cash_back_amount"'); ?>
			                </div>
			                <div class="form-group">
				                <?= lang('start_date', 'cash_back_start_date'); ?>
				                <?= form_input('cash_back_start_date', set_value('cash_back_start_date', $product->cash_back_start_date ? $this->rerp->hrsd($product->cash_back_start_date) : ''), 'class="form-control tip date" id="cash_back_start_date"'); ?>
			                </div>
			                <div class="form-group">
				                <?= lang('end_date', 'cash_back_end_date'); ?>
				                <?= form_input('cash_back_end_date', set_value('cash_back_end_date', $product->cash_back_end_date ? $this->rerp->hrsd($product->cash_back_end_date) : ''), 'class="form-control tip date" id="cash_back_end_date"'); ?>
			                </div>
		                </div>
	                </div>
                    ...
                    <?php if ($Settings->tax1) { ?>
                        <div class="form-group all">
                            ...
                            <?= form_dropdown('tax_rate', $tax_rates, (isset($_POST['tax_rate']) ? $_POST['tax_rate'] : ($product ? $product->tax_rate : $Settings->default_tax_rate)), 'class="form-control select" id="tax_rate" placeholder="' . lang('select') . ' ' . lang('product_tax') . '" style="width:100%"'); ?>
                        </div>
                        <div class="form-group all">
                    ...
                    <div class="form-group standard">
                        ...
                        <div
                            class="input-group"> <?= form_input('alert_quantity', (isset($_POST['alert_quantity']) ? $_POST['alert_quantity'] : ($product ? $this->rerp->formatDecimal($product->alert_quantity) : '')), 'class="form-control tip" id="alert_quantity"') ?>
	                ...
	                <div class="form-group row product-gallery" id="main_mage">
		                <?php if ( ! empty( $product->image ) ) { ?>
		                <div class="product-gallery-image col-md-3">
			                <a href="#" class="remove-image">
				                <i class="fa-fw fa fa-trash-o"></i>
			                </a>
			                <img src="<?php echo site_url( 'assets/uploads/thumbs/' . $product->image ); ?>" alt="">
		                </div>
		                <?php } ?>
	                </div>
                    <div class="form-group all">
	                ...
	                <div class="form-group row product-gallery" id="additional_image">
		                <?php foreach( $product_images as $img ) { ?>
		                <div class="product-gallery-image col-md-3" data-image_id="<?php echo $img->id; ?>">
			                <a href="#" class="remove-image">
				                <i class="fa-fw fa fa-trash-o"></i>
			                </a>
			                <img src="<?php echo site_url( 'assets/uploads/thumbs/' . $img->photo ); ?>" alt="">
		                </div>
		                <?php } ?>
	                </div>
                </div>
                            ...
                            if (!empty($warehouses) || !empty($warehouses_products)) {
                                ...
                                if (!empty($warehouses_products)) {
                                    foreach ($warehouses_products as $wh_pr) {
                                        echo '<span class="bold text-info">' . $wh_pr->name . ': <input type="hidden" value="' . $this->rerp->formatDecimal($wh_pr->quantity) . '" id="vwh_qty_' . $wh_pr->id . '"><span class="padding05" id="rwh_qty_' . $wh_pr->id . '">' . $this->rerp->formatQuantity($wh_pr->quantity) . '</span>' . ($wh_pr->rack ? ' (<span class="padding05" id="rrack_' . $wh_pr->id . '">' . $wh_pr->rack . '</span>)' : '') . '</span><br>';
                                    }
                                }
                        ...
                        <div class="well well-sm">
                            ...
                            <table class="table table-bordered table-condensed table-striped" style="<?= $this->input->post('attributes') || $product_options ? '' : 'display:none;'; ?> margin-top: 10px;">
                                ...
                                foreach ($product_options as $option) {
                                    echo '<tr><td class="col-xs-3"><input type="hidden" name="attr_id[]" value="' . $option->id . '"><span>' . $option->name . '</span></td><td class="code text-center col-xs-3"><span>' . $option->wh_name . '</span></td><td class="quantity text-center col-xs-2"><span>' . $this->rerp->formatQuantity($option->wh_qty) . '</span></td><td class="price text-right col-xs-2">' . $this->rerp->formatMoney($option->price) . '</td></tr>';
                                } ?>
                            </tbody>
                            </table>
                            <?php
                                            ...
                                            if ( $this->input->post( 'attributes' ) ) {
	                                            ...
	                                            for ( $r = 0; $r <= $a; $r ++ ) {
	                                                if ( isset( $_POST['attr_name'][ $r ] ) && ( isset( $_POST['attr_warehouse'][ $r ] ) || isset( $_POST['attr_quantity'][ $r ] ) ) ) {
                                                        echo '<tr class="attr">
	                                                        <td>
		                                                        <input type="hidden" name="attr_name[]" value="' . $_POST['attr_name'][$r] . '">
		                                                        <span>' . $_POST['attr_name'][$r] . '</span>
	                                                        </td>
	                                                        <td class="code text-center">
	                                                            ...
	                                                            <span>' . (isset($_POST['attr_wh_name'][$r]) ? $_POST['attr_wh_name'][$r] : '') . '</span>
                                                            </td>
	                                                        <td class="quantity text-center">
	                                                            <input type="hidden" name="attr_quantity[]" value="' . $_POST['attr_quantity'][$r] . '">
	                                                            <span>' . $_POST['attr_quantity'][$r] . '</span>
                                                            </td>
	                                                        <td class="price text-right">
	                                                            <input type="hidden" name="attr_price[]" value="' . $_POST['attr_price'][$r] . '">
	                                                            <span>' . $_POST['attr_price'][$r] . '</span>
                                                            </td>
                ...
                <div class="col-md-12">
	                ...
	                <div class="row custom_meta"><?php render_custom_fields( $custom_fields, $custom_values ); ?></div>
                    <div class="form-group all">
                    ...
                    <div class="form-group all">
                        ...
                        <?= form_textarea('details', (isset($_POST['details']) ? $_POST['details'] : ($product ? $product->details : '')), 'class="form-control" id="product_details_for_invoice"'); ?>
                    </div>
...
<?php if( isset( $this->themeInfos['product_custom'], $this->themeInfos['product_custom']['fields'] ) ) { ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        <?= isset($_POST['cash_back']) || $product->cash_back ? '$("#cash_back").iCheck("check");' : '' ?>
	    <?= isset($_POST['cash_back_percentage_on']) || $product->cash_back_percentage > 0 ? '$("#cash_back_percentage_on").iCheck("check");' : '' ?>

        $('#promotion').on('ifChecked', function (e) {
            $('#promo').slideDown();
        });
        $('#cash_back').on('ifChecked', function (e) {
            $('#cash_back_settings').slideDown();
        });
        $('#promotion').on('ifUnchecked', function (e) {
            $('#promo').slideUp();
        });
        $('#cash_back').on('ifUnchecked', function (e) {
            $('#cash_back_settings').slideUp();
        });

        //cash back percentage is on or off
        $('#cash_back_percentage_on').on('ifChecked', function (e) {

            $('#cash_back_percentage_field').slideDown();
            $('#cash_back_amount_field').slideUp();
            $('#cash_back_percentage').val('<?php echo ($product->cash_back_percentage ? $this->rerp->formatDecimal($product->cash_back_percentage) : '') ?>');
            $('#cash_back_amount').val('0').trigger('change');
        });
        $('#cash_back_percentage_on').on('ifUnchecked', function (e) {
            $('#cash_back_percentage_field').slideUp();
            $('#cash_back_amount_field').slideDown();
            $('#cash_back_amount').val('<?php echo ($product->cash_back_amount ? $this->rerp->formatDecimal($product->cash_back_amount) : '') ?>');
            $('#cash_back_percentage').val('0').trigger('change');
        });
        ...
        function bsAlert( type, message, el ) {
        	el = el || $('.bs-alerts');
        	$(`
			...
			`).prependTo( el );
	        scrollTo( 0, ( el.offset().top - 90 ) );
        }
        
	    $(document).ready( function() {
	    	
		    $(document).on( 'click', '.product-gallery-image .remove-image', function(e) {
			    e.preventDefault();
			    var img = $(this).closest( '.product-gallery-image' ),
				    data = { id: <?php echo $product->id; ?>, token: site.csrf_token_value, };
			    if ( img.data('image_id') ) {
				    data.img_id = parseInt( img.data( 'image_id' ) );
			    }
			    if ( confirm( '<?php echo lang( 'remove_this_image' ); ?>' ) ) {
				    $.ajax({
					    type: "POST",
					    url: "<?= admin_url('products/ajax_remove_product_image') ?>/",
					    data: data,
					    success: function ( response ) {
						    if ( 'success' === response.status ) {
							    img.remove();
						    }
						    bsAlert( response.status, response.message );
					    }
				    });
			    }
		    } );
	    ...
	    function imageViewer( imageData, gallery, replace ) {
		    replace = replace || false;
		    var reader = new FileReader();
		    reader.onload = function ( frEvent ) {
		    	var img = `<div class="product-gallery-image col-md-3 new"><img src="${frEvent.target.result}" alt=""></div>`;
		    	if ( ! replace ) {
        ...
        $("input#images, #product_image").on('change.bs.fileinput', function () {
	        var ag = '',
		        replace = false;
        	if ( 'product_image' === $(this).attr( 'id' ) ) {
        		ag = $('#main_mage');
		        replace = true;
		        ag.empty();
	        } else {
	        ...
	        for ( var x = 0; x < result.length; x++ ) {
	        	imageViewer( result[x], ag, replace );
            }
        });
        ...
        $('#addAttributes').click(function (e) {
            ...
            for (var i in attrs) {
                if (attrs[i] !== '') {
                    __ATTRS__ = `<tr class="attr">
					   <td><input type="hidden" name="attr_name[]" value="${attrs[i]}"><span>${attrs[i]}</span></td>
					   <td class="code text-center"><input type="hidden" name="attr_warehouse[]" value=""><span></span></td>
					   <td class="quantity text-center"><input type="hidden" name="attr_quantity[]" value="0"><span></span></td>
					   <td class="price text-right"><input type="hidden" name="attr_price[]" value="0"><span>0</span></span></td>
					   <td class="text-center"><i class="fa fa-times delAttr"></i></td>
					</tr>`;
                    $('#attrTable').show().append(__ATTRS__);
                }
            }
        });
    ...
    $(document).ready(function () {
        ...
        $('#supplier_price').val("<?= $this->rerp->formatDecimal($product->supplier1price); ?>");
        ...
        $('#supplier_2_price').val("<?= $this->rerp->formatDecimal($product->supplier2price); ?>");
        $('#supplier_2_part_no').val("<?= $product->supplier2_part_no; ?>");
        <?php } ?>
        <?php if ($product->supplier3) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_3', "<?= $product->supplier3; ?>");
        $('#supplier_3_price').val("<?= $this->rerp->formatDecimal($product->supplier3price); ?>");
        ...
        $('#supplier_4_price').val("<?= $this->rerp->formatDecimal($product->supplier4price); ?>");
        ...
        $('#supplier_5_price').val("<?= $this->rerp->formatDecimal($product->supplier5price); ?>");
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">
                    <span aria-hidden="true"><i class="fa fa-2x">&times;</i></span><span class="sr-only">Close</span>
                </button>
