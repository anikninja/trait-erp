                                        ...
                                        <div class="form-group">
                                            ...
                                            if ($variants) {
                                                foreach ($variants as $variant) {
                                                    $opts[$variant->id] = $variant->name . ($variant->price > 0 ? ' (+' . $this->rerp->convertMoney($variant->price, true, false) . ')' : ($variant->price == 0 ? '' : ' (+' . $this->rerp->convertMoney($variant->price, true, false) . ')'));
                                                }
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-striped dfTable table-right-left">
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td><?= lang('price'); ?></td>
                                                        <td><?= $this->rerp->convertMoney(isset($product->special_price) ? $product->special_price : $product->price); ?></td>
                                                    </tr>
                                                    <?php
                                                    } ?>
                                                    ...
                                                    if ($product->promotion) {
                                                        echo '<tr><td>' . lang('promotion') . '</td><td><strong>' . $this->rerp->convertMoney($product->promo_price) . '</strong><br>' . ($product->start_date && $product->start_date != '0000-00-00' ? lang('start_date') . ': <strong>' . $this->rerp->hrsd($product->start_date) . '</strong><br>' : '') . ($product->end_date && $product->end_date != '0000-00-00' ? lang('end_date') . ': <strong>' . $this->rerp->hrsd($product->end_date) . '</strong>' : '') . '</td></tr>';
                                                    }
                                                    ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <!-- <td><?= $this->rerp->formatQuantity($warehouse->quantity); ?></td> -->
                                                    </tr>
                                                    <?php
                                                    } ?>
                                                    ...
                                                    <?php if (!empty($options)) {
                                                        foreach ($options as $option) {
                                                            if ($option->wh_qty != 0) {
                                                                ...
                                                                // echo '<tr><td>' . lang('in_stock') . ': ' . $this->rerp->formatQuantity($option->wh_qty) . '</td>';
                                                                if (!$shop_settings->hide_price) {
                                                                    echo '<tr><td>' . lang('in_stock') . ': ' . ($option->wh_qty ? lang('yes') : lang('no')) . '</td>';
                                                                    echo '<td>' . lang('price') . ': ' . $this->rerp->convertMoney((isset($product->special_price) ? $product->special_price : $product->price) + $option->price) . '</td>';
                                                                    echo '</tr>';
                                                                } else {
                            ...
                            <div class="details" style="transition: all 100ms ease-out 0s;">
                                ...
                                <div class="stats-container">
                                    ...
                                    <span class="product_price">
                                        <?php
                                        if ($fp->promotion) {
                                            echo '<del class="text-red">' . $this->rerp->convertMoney(isset($fp->special_price) && !empty($fp->special_price) ? $fp->special_price : $fp->price) . '</del><br>';
                                            echo $this->rerp->convertMoney($fp->promo_price);
                                        } else {
                                            echo $this->rerp->convertMoney(isset($fp->special_price) && !empty($fp->special_price) ? $fp->special_price : $fp->price);
                                        } ?>
                                    </span>
                                    <?php
                                } ?>
