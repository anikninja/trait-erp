    ...
    <div class="flex-center position-ref full-height">
        ...
        <div class="content">
            <div class="title m-b-md">
                <?= isset($msg) ? $msg : lang('site_offline'); ?>
            </div>

            <div class="links">
                <div class="m-b-md">
                    <a><?= isset($msg1) ? $msg1 : lang('visit_us_later'); ?></a><br>
                </div>
                <a href="<?= site_url('/'); ?>"><?= lang('home'); ?></a>
                <?= $this->rerp->logged_in() ? '<a href="'.shop_url('orders').'">'.lang('orders').'</a>' : '<a href="'.site_url('login').'">'.lang('login').'</a>'; ?>
                <?= ($this->rerp->logged_in() && (!$this->rerp->in_group('customer') && !$this->rerp->in_group('customer'))) ? '<a href="'.site_url('admin').'">'.lang('dashboard').'</a>' : ''; ?>
            </div>
        </div>
    </div>
</body>
