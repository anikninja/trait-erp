...
<html>
	<head>
	    ...
	    <link href="<?= base_url('themes/' . $Settings->theme . '/shop/assets/css/pdf/pdf.css'); ?>" rel="stylesheet">
	</head>
	<body>
	    <div id="wrap">
	        <div class="row">
	            <div class="col-lg-12">
	                    ...
	                    <div class="col-xs-5 pull-right">
	                        <div class="well well-sm">
	                            ...
	                            <table style="margin-bottom: 10px;">
	                                <tr>
	                                    <td><?= lang('ref'); ?> </td><td>: <strong><?= $inv->reference_no; ?></strong></td>
	                                </tr>
	                                <tr>
	                                    <td><?= lang('date'); ?></td><td>: <strong><?= $this->rerp->hrld($inv->date); ?></strong></td>
	                                </tr>
	                                <tr>
	                                    <td><?= lang('sale_status'); ?></td><td>: <strong><?= lang($inv->sale_status); ?></strong></td>
	                                </tr>
	                                <tr>
	                                    <td><?= lang('payment_status'); ?></td><td>: <strong><?= lang($inv->payment_status); ?></strong></td>
	                                </tr>
	                            </table>
	                        ...
	                        <table class="table table-bordered table-hover table-striped">
		                                ...
		                                if ( $Settings->tax1 && $inv->product_tax > 0 ) {
			                                echo '<td style="width: 90px; text-align:right; vertical-align:middle;">'
			                                     . ( $row->item_tax != 0 && $row->tax_code
					                                ...
					                                : '' )
			                                     . $this->rerp->formatMoney( $row->item_tax )
			                                     . '</td>';
		                                }
                                ...
                                if ( $return_rows ) {
	                                    ...
	                                    <tr class="warning">
	                                        ...
	                                        <?php
	                                        if ($Settings->tax1 && $inv->product_tax > 0) {
	                                            echo '<td style="text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 && $row->tax_code ? '<small>(' . $row->tax_code . ')</small>' : '') . ' ' . $this->rerp->formatMoney($row->item_tax) . '</td>';
	                                        }
	                                    if ($Settings->product_discount && $inv->product_discount != 0) {
	                            ...
	                            <tfoot>
	                                ...
	                                <?php if ($Settings->indian_gst) {
	                                    if ($inv->cgst > 0) {
	                                        ...
	                                        echo '<td style="text-align:right;">' . ($Settings->format_gst ? $this->rerp->formatMoney($cgst) : $cgst) . '</td></tr>';
	                                    }
	                                    if ($inv->sgst > 0) {
	                                        ...
	                                        echo '<td style="text-align:right;">' . ($Settings->format_gst ? $this->rerp->formatMoney($sgst) : $sgst) . '</td></tr>';
	                                    }
	                                    if ($inv->igst > 0) {
	                                        ...
	                                        echo '<td style="text-align:right;">' . ($Settings->format_gst ? $this->rerp->formatMoney($igst) : $igst) . '</td></tr>';
	                                    }
	                                } ?>
	                                <?php
	                                if ($return_sale) {
	                                    echo '<tr><td colspan="' . $col . '" style="text-align:right; font-weight:bold;">' . lang('return_total') . ' (' . $default_currency->code . ')</td>';
	                                    echo '<td style="text-align:right; font-weight:bold;">' . $this->rerp->formatMoney($return_sale->grand_total) . '</td></tr>';
	                                }
	                                if ($inv->surcharge != 0) {
	                                    echo '<tr><td colspan="' . $col . '" style="text-align:right; font-weight:bold;">' . lang('return_surcharge') . ' (' . $default_currency->code . ')</td>';
	                                    echo '<td style="text-align:right;">' . $this->rerp->formatMoney($inv->surcharge) . '</td></tr>';
	                                }
	                                ?>
	                                <?php if ($inv->order_discount != 0) {
	                                    echo '<tr><td colspan="' . $col . '" style="text-align:right; font-weight:bold;">' . lang('order_discount') . ' (' . $default_currency->code . ')</td>';
	                                    echo '<td style="text-align:right;">' . ($inv->order_discount_id ? '<small>(' . $inv->order_discount_id . ')</small> ' : '') . $this->rerp->formatMoney($return_sale ? ($inv->order_discount + $return_sale->order_discount) : $inv->order_discount) . '</td></tr>';
	                                }
	                                ?>
	                                <?php if ($Settings->tax2 && $inv->order_tax != 0) {
	                                    echo '<tr><td colspan="' . $col . '" style="text-align:right; font-weight:bold;">' . lang('order_tax') . ' (' . $default_currency->code . ')</td>';
	                                    echo '<td style="text-align:right;">' . $this->rerp->formatMoney($return_sale ? ($inv->order_tax + $return_sale->order_tax) : $inv->order_tax) . '</td></tr>';
	                                }
	                                ?>
	                                <?php if ($inv->shipping != 0) {
	                                    echo '<tr><td colspan="' . $col . '" style="text-align:right; font-weight:bold;">' . lang('shipping') . ' (' . $default_currency->code . ')</td>';
	                                    echo '<td style="text-align:right;">' . $this->rerp->formatMoney($inv->shipping) . '</td></tr>';
	                                }
	                                ?>
	                                <tr>
	                ...
	                <div class="col-xs-12">
	                    ...
	                    <?php } ?>
	                    <?php
	                    if ( 'bank' === $inv->payment_method && ! empty( $shop_settings->bank_details ) ) {
